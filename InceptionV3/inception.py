...
# Data transformations
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # InceptionV3 normalization values
    ])
    ...
    start_epoch = 1
    if resume:
        if os.path.exists("inception_checkpoint.pth.tar"):
            checkpoint = load_checkpoint("inception_checkpoint.pth.tar")
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch}/{num_epochs} [Training]", unit="batch") as tepoch:
            for images, labels in tepoch:
                ...
                # Forward pass
                outputs, aux_outputs = model(images)  # Inception has two outputs during training
        ...
        # Validation loop (only main outputs are used)
        model.eval()
        ...
        save_checkpoint(checkpoint, file_name="inception_checkpoint.pth.tar")
